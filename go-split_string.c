...
/**
 * splittings_string - splits a string into words specified by a delimiter(s),
...
char **splittings_string(char *str, char *separators, size_t *word_count)
{
	...
	setting_zeros(word_sizes, MAX_WORD_COUNT);
	no_of_words = words_countting(str, separators, word_sizes);
	...
	for (v = 0; v < no_of_words; v++)
	{
		...
		{
			while (is_a_delimiter(*str_ptr, separators))
				str_ptr++;
...
char **stringtow(char *str, char *d)
{
	...
	if (!d)
		d = " ";
	for (i = 0; str[i] != '\0'; i++)
		if (!is_a_delimiter(str[i], d) && (is_a_delimiter(str[i + 1], d) || !str[i + 1]))
			numwords++;
	if (numwords == 0)
		return (NULL);
	...
	{
		while (is_a_delimiter(str[i], d))
			i++;
		k = 0;
		while (!is_a_delimiter(str[i + k], d) && str[i + k])
			k++;
