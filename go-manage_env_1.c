...
/**
 * _settingsenv - Adds or updates an environment variable.
 * @info: Structure containing the environment variables.
 *		  Used to maintain constant function prototype.
...
int _settingsenv(info_s *info, char *var, char *value)
{
	char *buf = NULL;
	list_s *node;
	char *p;
	...
	while (node)
	{
		...
		p = starttings_with(node->str, var);
		if (p && *p == '=')
		{
	...
	add_node_endding(&(info->env), buf, 0);
	free(buf);
	info->env_changed = 1;
	return (0);
}
...
int _unsettingsenv(info_s *info, char *var)
{
	...
	while (node)
	{
		...
		if (p && *p == '=')
		{
			...
			info->env_changed = delettings_node_at_index(&(info->env), index);
